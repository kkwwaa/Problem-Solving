...
vi bfs(int sx, int sy){
    ...
    while (!q.empty()) {
        ...
        // определяем, какой символ в клетке, чтобы понять, какого типа корабль
        if (a[x][y]==1)
            find_s=true;
        else if (a[x][y]==2)
            find_x=true;
        
        // для просмотра всех близлежащих клеток
        int dx[] = {0,0,-1,1};
        int dy[] = {-1,1,0,0};

        forn(i, 0,4){
            // вычисляем новые координаты ячейки поблизости
            int nx = x+dx[i];
            int ny = y+dy[i];

            if (nx>=0 && ny>=0 && nx<n && ny<m && !used[nx][ny] && a[nx][ny]>0){
                used[nx][ny] = true;
                q.push({nx,ny});
            }
        }
    ...
    // возвращаем счетчик по типам кораблей
    if (find_s && find_x)
        return {0,1,0};
    else if (find_s)
        return {1,0,0};
    ...
    forn(i, 0, n){
        forn(j, 0, m){
            if (a[i][j]>0 && !used[i][j]) {
                ...
                forn(e,0,3) ans[e]+=retur[e];// увеличиваем ответ
            }
        }
    }
    fout(ans);
