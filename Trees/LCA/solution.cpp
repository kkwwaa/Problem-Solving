...
void dfs(int v, int p){
    tin[v] = ++timer;// вход в вершину
    up[v][0] = p; // предок на 2^0 от v

    forn(lvl, 1, 20){
        up[v][lvl] = up[up[v][lvl-1]][lvl-1]; // двоичные прыжки, up[v][lvl] -> предок в-ны v на 2^lvl от v
    }

    forn(i, 0, edges[v].size()) { // проходимся дфс по вершинке
        int to = edges[v][i].first; // достаем смежную вершинку
        int w = edges[v][i].second; // расстояние от родителя
        if (to == p) // в смежных есть предок, его не трогаем
            continue;
        dist[to] = dist[v] + w; // расстояние от корня до вершины
        dfs(to, v); // рекурсивный вызов для потомка
    }
}
...
int lca (int a, int b){
    ...
    for (int i=19; i>=0; i--){
        int pred = up[a][i];// прыжок на 2^i от а
        if (isParent(pred,b)) // если предок, то ищем дальше
            continue;
        a = pred; // если нет, то прыгаем на эту вершинку (к-ая не предок)
    }
    return up[a][0]; // в итоге, а - вершинка предок для одного, но не предок для другого -> вершинка чуть выше преодок для обеих
}
...
void solve() {
    ...
    while(q--){ // обрабатываем запросы
        ...
        cin >> u>> v;
        int LCA = lca(u,v);
        cout << dist[u] + dist[v] - 2*dist[LCA] << endl; // сумма расстояний от корня до двух вершин, минус повторяющаяся часть (рис)
    }
}
