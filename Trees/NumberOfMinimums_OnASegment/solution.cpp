...
// Построение дерева
void build(int v, int tl, int tr, const vi& a, vi& cnt) {
    if (tl == tr) {
        tree[v] = a[tl];
        cnt[v] = 1;
        return;
    ...
    int tm = (tl + tr) / 2;
    build(v * 2, tl, tm, a, cnt); // левый ребенок
    build(v * 2 + 1, tm + 1, tr, a, cnt); // правый ребенок
    int resLeft = tree[v*2];
    int resRight = tree[v*2+1];
    if (resLeft == resRight) {
        tree[v] = resLeft;
...
// Обновление значения
void update(int v, int tl, int tr, int pos, int val, vi& cnt) {
    if (tl == tr) {
        tree[v] = val;
        return;
    }
    int tm = (tl + tr) / 2;
    if (pos <= tm)
        update(v * 2, tl, tm, pos, val, cnt);
    else
        update(v * 2 + 1, tm + 1, tr, pos, val, cnt);
    int resLeft = tree[v*2];
    int resRight = tree[v*2+1];
    if (resLeft == resRight) {
        tree[v] = resLeft;
...
pair<ll, ll> get(int v, int tl, int tr, int l, int r, const vi& cnt) {
    if (l > r)
        return {INF, 0};
    if (tl == l && tr == r)
        return {tree[v], cnt[v]};
    int tm = (tl + tr) / 2;
    pair<ll, ll> resLeft = get(v * 2, tl, tm, l, min(r, tm), cnt);
    pair<ll, ll> resRight = get(v * 2 + 1, tm + 1, tr, max(l, tm + 1), r, cnt);
    if (resRight.first == resLeft.first) return {resLeft.first, resLeft.second+resRight.second};
    else if (resLeft.first < resRight.first) return {resLeft.first, resLeft.second};
    else return {resRight.first, resRight.second};
}
...
int solve() {
    ...
    forn(i,0,m) {
        ...
        if (type == 1) {
            // обновление: a[x] = y
            update(1, 0, n - 1, x, y, cnt);
        } else {
            pair<ll, ll> res = get(1, 0, n - 1, x, y - 1, cnt);
            // запрос от x до y-1
            cout << res.first <<" "<< res.second << en;
        }
    }
